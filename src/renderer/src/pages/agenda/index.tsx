...
export async function handleDeleteEvent(id: string, refresh, setRefresh): Promise<void> {
  try {
...
export async function handleUpdateEvent(data: Agenda, refresh, setRefresh): Promise<void> {
  try {
...
export async function handleSubmitEvent(data: Agenda, refresh, setRefresh): Promise<void> {
  try {
    await window.api.Database(
      `
...
export async function fetchEventData(all: boolean, date: Dayjs): Promise<Agenda[]> {
...
async function fetchEvents(date: Dayjs): Promise<Event[]> {
    ...
    <div className="right-content-secondary">
      <form
        ...
        onSubmit={async (e) => {
          ...
          const data = {
            ...
            information: (e.currentTarget.elements as unknown as { nameInput: { value: string } })
              .nameInput.value
          }
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h1 className="modal-title fs-5" id="agendaModalLabel">
                  Event
                </h1>
                <button
                  type="button"
      ...
      <div className="area-first overflow-y-auto">
        ...
        <div className="w-100 d-flex flex-column align-items-center pt-4">
          ...
          <button
            type="button"
            className="btn btn-sm text-center secondary text-white mt-1"
          ...
          {agenda.map((data, index) => {
            ...
            return (
              <div
                ...
                onClick={() => {
                  setId(data.id)
                  if (nameRef.current) nameRef.current.value = data.information
              ...
              >
                <div className="ms-5 mt-5 ps-5 d-flex gap-4" style={{ height: '50px' }}>
                  ...
                  <div
                    className="d-flex mt-auto mb-auto ps-1 rounded-4"
                    style={{
                      height: '38px',
                      width: '4px',
                  ...
                  <div className="d-flex flex-column mt-auto mb-auto">
                    ...
                    <span className="mt-1">{abbreviateString(data.information, 25)}</span>
                  </div>
                </div>
              </div>
            )
