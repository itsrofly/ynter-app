...
function NewReceipt({ table, refresh, setRefresh, form, handleSubmit }: {
    table: "revenue" | "expense",
    refresh: boolean, setRefresh: any,
    form: FormState,
    handleSubmit: any
}) {
                            ...
                            <div className="modal-content">
                                ...
                                <div className="modal-body">
                                    <div className="input-group mb-3">
                                        ...
                                        <input type="text" className="form-control" placeholder="Ynter Premium" aria-label="Name" aria-describedby="basic-addon1"
                                            value={nameValue} onChange={(e) => setNameValue(e.target.value)} required />
                                    </div>

                                    <div className="input-group mb-3">
                                        <span className="input-group-text" style={{ width: "100px" }}>Description</span>
                                        <textarea className="form-control" placeholder="Optional" aria-label="With textarea"
                                            value={descValue} onChange={(e) => setDescValue(e.target.value)}></textarea>
                                    </div>

                                    <div className="d-flex flex-row  mb-5">
                                        <div className="input-group">
                                            <input type="number" className="form-control" placeholder="1000" aria-label="Amount"
                                                value={amountValue} onChange={(e) => setAmountValue(e.target.value)} required />
                                            <span className="input-group-text">$</span>
                                        </div>

                                        <div className="input-group ps-4">
                                            <input type="date" className="form-control" aria-label="Date"
                                                value={dateValue} onChange={(e) => setDateValue(e.target.value)} required />
                                        </div>
                                    </div>
                                    ...
                                    <div className="input-group mb-4">
                                        <select className="form-select custom-select" style={{ height: "38px" }}
                                            value={typeValue} onChange={(e) => setTypeValue(e.target.value)} >
                                            <option className="custom-option" value={0}>Bank transfer</option>
                                            <option className="custom-option" value={1}>Card</option>
                                            <option className="custom-option" value={2}>Cash</option>
                                        </select>
                                        <input type="text" className="form-control w-50" placeholder="Bank name" aria-label="Bank Name"
                                            value={bankValue} onChange={(e) => setBankValue(e.target.value)} required />
                                    </div>

                                    <div className="input-group mb-3">
                                        <span className="input-group-text" id="basic-addon1">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="18px" height="18px" fill="currentColor" className="bi bi-paperclip" viewBox="0 0 16 16">
                                                <path d="M4.5 3a2.5 2.5 0 0 1 5 0v9a1.5 1.5 0 0 1-3 0V5a.5.5 0 0 1 1 0v7a.5.5 0 0 0 1 0V3a1.5 1.5 0 1 0-3 0v9a2.5 2.5 0 0 0 5 0V5a.5.5 0 0 1 1 0v7a3.5 3.5 0 1 1-7 0z" />
                                            </svg>
                                        </span>
                                        <button type="button" className="border btn form-control border text-start"
                                            onClick={async () => {
                                                const result = await window.api.showGetFile();

                                                if (result.filesize > 100)
                                                    window.api.showError("Couldn't add the file, it's too large.\nMax size is 100mb.")
                                ...
                                <div className="modal-footer">
                                    <div className="form-check d-flex align-items-center me-auto">
                                        <input className="form-check-input mb-2" type="checkbox" id="flexCheckDefault"
                                            checked={recuValue}
                                    ...
                                    {idValue && <button type="button" className="btn btn-danger" data-bs-dismiss="modal"
                                        onClick={() => {
