export const PlaidRevenueCategory = (personal_finance_category: {
  detailed: string
  primary: string
}): number => {
  /* 
  ...
  if (list[primary] && list[primary][detailed] != undefined) return list[primary][detailed]
  return 4
}

export const PlaidExpenseCategory = (personal_finance_category: {
  detailed: string
  primary: string
}): number => {
  /* 
(
  ...
  if (list[primary] && list[primary][detailed] != undefined) return list[primary][detailed]
  else if (list[primary] && typeof list[primary] === 'number') return list[primary]
  return 12
}
