...
export const BankType = (index: number): { Icon: JSX.Element; Label: string } => {
  // Paymeny method
  switch (index) {
    case 0:
      return {
...
export async function handleSubmitReceipt(values: FormState, table: string): Promise<void> {
  try {
...
export async function handleUpdateReceipt(values: FormState, table: string): Promise<void> {
  try {
    const query = `
          UPDATE ${table}
          SET name = ?, 
...
export async function handleDeleteReceipt(id: string, table: string): Promise<void> {
  try {
    const query = `
          DELETE FROM ${table}
          WHERE id = ${id};
...
export async function updateRecurring(table: 'revenue' | 'expense'): Promise<void> {
  try {
